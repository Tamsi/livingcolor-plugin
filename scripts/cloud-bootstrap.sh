...
warmup_jira_mcp() {
  ...
  if ! python3 -c "import json; d=json.load(open('/tmp/lc-jira-connect.json')); raise SystemExit(0 if d.get('ok') else 1)" 2>/dev/null; then
    log "WARNING: LivingColor Jira connect returned ok=false"
    return 1
  fi
